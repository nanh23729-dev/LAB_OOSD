                ...
                try
                {
                    foreach (string maSach in unique)
                    {
                        using (SqlCommand check = new SqlCommand(
                            "SELECT SoLuongHienCo FROM DauSach WITH (UPDLOCK,HOLDLOCK) WHERE MaDauSach=@Ma", cn, tx))
                        {
                    ...
                    foreach (string maSach in unique)
                    {
                        ...
                        using (SqlCommand insCt = new SqlCommand(
                            "INSERT INTO ChiTietPhieuMuon(MaChiTiet,MaPhieuMuon,MaDauSach) VALUES(@MaCT,@MaPM,@MaSach)", cn, tx))
                        {
                        ...
                        using (SqlCommand update = new SqlCommand(
                            "UPDATE DauSach SET SoLuongHienCo=SoLuongHienCo-1 WHERE MaDauSach=@Ma AND SoLuongHienCo>0", cn, tx))
                        {
